"""Idempotency-Key dependency for retryable POST routes.

Lift into app/core/idempotency.py. Requires migration 005 (idempotency_keys).

Usage:
    @router.post("", status_code=201)
    async def create_lead(lead: LeadIn, idem: IdemGuard = Depends(idem_guard)) -> LeadOut:
        if idem.replay is not None:
            return idem.replay                      # stored response, no side effects
